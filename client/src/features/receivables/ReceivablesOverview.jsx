...
export default function ReceivablesOverview() {
  ...
  return (
    <>
      <div className="kpi-row">
        <StatCard label="Total outstanding" countTo={data.totalOutstanding} format={ngnWhole} icon={<AlertCircle size={17} />} iconTone={data.totalOutstanding > 0 ? "neg" : "neutral"} hint="Across all open balances" />
        <StatCard label="Collected this month" countTo={data.collectedThisMonth} format={ngnWhole} tone={data.collectedThisMonth > 0 ? "pos" : undefined} iconTone="pos" icon={<HandCoins size={17} />} hint="Payments received this month" />
